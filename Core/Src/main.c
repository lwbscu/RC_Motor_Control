...
static void MX_TIM2_Init(void)
{
    ...
    /* =================================================================
     * 关键修改：
     * 原 Prescaler = 16999, Period = 99 -> 100Hz (10ms)
     * 新 Prescaler = 1699,  Period = 99 -> 1000Hz (1ms)
     * 170,000,000 / (1699 + 1) / (99 + 1) = 1000 Hz
     * ================================================================= */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == TIM2) {
        ...
        system_counter++; // 每1ms增加1

        // LED1快闪 - 每500ms闪烁一次 (500 * 1ms)
        if (++led_counter >= 500) {
            led_counter = 0;
            LED1_TOGGLE();  // LED1快闪，表示程序在运行
        }

        // LED2慢闪 - 每2秒闪烁一次 (2000 * 1ms)
        if (system_counter % 2000 == 0) {
            LED2_TOGGLE();  // LED2慢闪，表示系统正常
        }
         ...
         * 关键修改：
         * 原 10 * 10ms = 100ms
         * 新 20 * 1ms  = 20ms (提高发送速度，便于调参)
         * ================================================================= */
        if (++vofa_send_counter >= 20) {
            vofa_send_counter = 0;
            VOFA_SendData();
        }
    }
